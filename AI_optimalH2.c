...
// defining a structure of a node to store the sliding puzzle,its cost(total no of misplaced tiles = g(n)),level h(n),blank block coordinates
typedef struct Node
{
    struct Node* parent;
    int mat[N][N];
...
//function for finding total no of misplaced tiles
